...
4.  **Service (Servicio)**: Es el "experto/cerebro". Aquí están las reglas del negocio, cálculos, llamadas a APIs externas (Steam/RAWG).
...
-   **`server.ts`**: El punto de entrada principal. Aquí arranca la aplicación, se conecta a la base de datos y se inician los servidores.
...
-   **`validators/`**: Reglas de validación. Por ejemplo: "El email debe tener un @", "La contraseña mínimo 6 caracteres".
...
-   **`middleware/`**: Funciones que se ejecutan _antes_ de llegar al controlador.
    -   `auth.ts`: Verifica si el usuario está logueado (JWT).
    -   `roles.ts`: Verifica si es Admin.
-   **`utils/`**: Herramientas genéricas que se usan en varios sitios (ej. formatear fechas, logs, encriptar contraseñas).
...
-   **Docker**: Permite que el proyecto corra igual en tu PC que en el de tu compañero, usando "contenedores" aislados.

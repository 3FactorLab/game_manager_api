...
export const login = asyncHandler(async (req: Request, res: Response) => {
    ...
    const { token, refreshToken, user } = await loginUser(loginData.email, loginData.password, ipAddress);
...
export const refreshToken = asyncHandler(async (req: Request, res: Response) => {
    ...
    const ipAddress = req.ip;
    const { token: newToken, refreshToken: newRefreshToken, user } = await refreshTokenService(token, ipAddress);
    res.status(200).json({

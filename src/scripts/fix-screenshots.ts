    ...
    try {
        ...
        if (!game) {
            ...
            const gamesMissingScreenshots = await Game.find({
                $or: [{ "assets.screenshots": { $exists: false } }, { "assets.screenshots": { $size: 0 } }],
                rawgId: { $ne: null },
            }).limit(10);

            logger.info(`Found ${gamesMissingScreenshots.length} other games with missing screenshots.`);
...
async function updateGame(game: any) {
    ...
    try {
        ...
        if (details.screenshots && details.screenshots.length > 0) {
            ...
            if (!game.image && details.image) {
                game.image = details.image;
                game.assets.cover = details.image;
            }
            // Also ensure title is correct if it was weird
            if (game.title === "common.free") {
